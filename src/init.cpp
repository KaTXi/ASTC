...
// Copyright (c)      2014 The AsturCoin developers
...
void Shutdown()
{
    ...
    RenameThread("asturcoin-shutoff");
    ...
    if (pwalletMain)
        bitdb.Flush(false);
    GenerateAsturCoins(false, NULL, 0);
#endif
    StopNode();
...
std::string HelpMessage(HelpMessageMode hmm)
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: asturcoin.conf)") + "\n";
    strUsage += "  -datadir=<dir>         " + _("Specify data directory") + "\n";
    strUsage += "  -testnet               " + _("Use the test network") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: asturcoind.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 71906 or testnet: 71913)") + "\n";
    ...
    strUsage += "\n" + _("SSL options: (see the AsturCoin Wiki for SSL setup instructions)") + "\n";
...
{
    RenameThread("asturcoin-loadblk");
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (mapArgs.count("-change"))
    {
        BOOST_FOREACH(std::string strChange, mapMultiArgs["-change"]) {
            CAsturCoinAddress address(strChange);
            CKeyID keyID;
...
#endif
    // Make sure only a single AsturCoin process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. AsturCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("AsturCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of AsturCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart AsturCoin to complete") << "\n";
    ...
    if (pwalletMain)
        GenerateAsturCoins(GetBoolArg("-gen", false), pwalletMain, GetArg("-genproclimit", -1));
#endif
