    ...
    transactionView(0)
{
	...
	QAction *viewOnAsturchain = new QAction(tr("Show transaction on AsturCoin Explorer"), this);
	...
	contextMenu->addSeparator();
	contextMenu->addAction(viewOnAsturchain);
	...
	connect(viewOnAsturchain, SIGNAL(triggered()), this, SLOT(viewOnAsturchain()));
}
...
void TransactionView::changedAmount(const QString &amount)
{
    ...
    if(AsturCoinUnits::parse(model->getOptionsModel()->getDisplayUnit(), amount, &amount_parsed))
    {
...
void TransactionView::viewOnAsturchain()
{
    QModelIndexList selection = transactionView->selectionModel()->selectedRows();
    if(!selection.isEmpty())
    {
        QString format("http://explorer.asturcoin.org/tx/");
