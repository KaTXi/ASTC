...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("AsturCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AsturCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AsturCoinMiner(CWallet *pwallet)
{
    printf("AsturCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("asturcoin-miner");
    ...
    {
        printf("AsturCoinMiner terminated\n");
        throw;
    }
}

void GenerateAsturCoins(bool fGenerate, CWallet* pwallet, int nThreads)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AsturCoinMiner, pwallet));
}
