...
#if defined(NDEBUG)
# error "AsturCoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "AsturCoin Signed Message:\n";
...
//TODO: this isn't identical to asturcoin reference client.
...
int64_t GetAsturCoinSubsidy(int nHeight) {
...
int64_t GetBlockValue(int nHeight, int64_t nFees, uint256 prevHash)
{
   ...
   if(nHeight < nDiffChangeTarget) {
      ...
      nSubsidy = GetAsturCoinSubsidy(nHeight);
   }
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // AsturCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
void ThreadScriptCheck() {
    RenameThread("asturcoin-scriptch");
...
bool ConnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // BIP16 was always active in AsturCoin
...
{
    // AsturCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
